      ...
      <div className="container relative z-10">
          ...
          {dict.plans.map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-base sm:text-sm text-muted-foreground mb-5">{plan.data} · {plan.validity}</p>
                <div className="mb-6">
                  <span className="font-display font-medium text-3xl text-white">{formatVnd(priceVnd)}</span>
                  <span className="text-base  sm:text-sm text-muted-foreground ml-2">{pricePerGbVnd}</span>
                </div>
                <Button
        ...
        <div className="glass-card p-8 md:p-12">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="font-display font-medium text-[1.7rem] sm:text-2xl text-white mb-2">{dict.allPlansTitle}</h3>
            ...
            <div className="grid sm:grid-cols-2 gap-3.5">
              {dict.benefits.map((benefit) => (
                <div key={benefit} className="flex items-center gap-2.5">
                  ...
                  <span className="text-base sm:text-sm text-white/80">{benefit}</span>
                </div>
              ))}
            </div>
          </div>
